...
class FrameLogger(object):
	...
	def service(self):
		if self.logging:
			...
			filename = "{:05d}-{:08.2f}-{:03d}.jpg".format(self.frames, time.monotonic() - self.start_time, self.frame_count)
			capture.frame(self.window, os.path.join(self.directory, filename), (640,360), subsampling = 0, quality = 100, format = "JPEG")
			self.frames += 1
			self.frame_count += 1
